...
(function($) {
    ...
    function Plugin($container, options) {
        ...
        var self = this
        ,   $movimiento = $container.find(".movimiento:first")
        ,   $contenedor_imagenes = $container.find(".contenedor_imagenes:first")
        ,   $slides = null
        ...
        this.update = function() {
            $contenedor_imagenes.find(".mirrored").remove();

            $slides = $contenedor_imagenes.children();
            viewportSize = $movimiento[0]["offset" + sizeLabel];
            slideSize = $slides.first()["outer" + sizeLabel](true);
            self.slidesTotal = $slides.length;
            self.slideCurrent = self.options.start || 0;
            slidesVisible = Math.ceil(viewportSize / slideSize);

            $contenedor_imagenes.append($slides.slice(0, slidesVisible).clone().addClass("mirrored"));
            $contenedor_imagenes.css(sizeLabel.toLowerCase(), slideSize * (self.slidesTotal + slidesVisible));
        ...
        this.move = function(index) {
            ...
            if(slideIndex < 0) {
                ...
                $contenedor_imagenes.css(posiLabel, -(self.slidesTotal) * slideSize);
            }

            if(slideIndex > self.slidesTotal) {
                self.slideCurrent = slideIndex = 1;
                $contenedor_imagenes.css(posiLabel, 0);
            }
